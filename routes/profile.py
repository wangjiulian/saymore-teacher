...
def index():
    ...
    try:
        ...
        return render_template('profile/view.html', teacher=teacher)
...
def edit():
    ...
    try:
        ...
        if request.method == 'GET':
            if teacher.subjects.count() > 0:
                ...
                current_app.logger.info(
                    f"为教师 {teacher.name} 设置了 {len(form.subjects.data)} 个选中学科: {form.subjects.data}")

            # 记录选择框中所有可用的学科选项
            current_app.logger.info(f"学科选择框中共有 {len(form.subjects.choices)} 个选项")
            for choice_id, choice_text in form.subjects.choices:
                current_app.logger.info(f"学科选项: ID={choice_id}, 文本={choice_text}")
        ...
        return render_template('profile/edit.html', form=form, teacher=teacher)
    except Exception as e:
